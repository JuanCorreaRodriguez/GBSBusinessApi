...
public class MediaController : ControllerBase
{
    ...
    public async Task<Result<List<MediaMinDto>>> GetAllMedia([FromQuery] QueryMedia queryMedia)
    {
        ...
        return medias.Count == 0
            ? Result<List<MediaMinDto>>.Failure(StatusErrors.NoContent)
    ...
    {
        if (id == "")
            return Result<MediaMinDto>.Failure(StatusErrors.DtoError);

        var media = await _mediaService.GetMediaAsync(id);
        return media == null
            ? Result<MediaMinDto>.Failure(StatusErrors.NoContent)
    ...
    {
        if (!ModelState.IsValid)
            Result<MediaMinDto>.Failure(StatusErrors.DtoError);
    ...
    {
        if (id == "")
            Result<Global>.Failure(StatusErrors.DtoError);
        ...
        return result
            ...
            : Result<Global>.Failure(StatusErrors.NotFound);
    }
}